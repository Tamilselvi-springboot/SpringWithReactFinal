...
public class StepDefinition {
	...
	public void testing_environment_is_set_up_for(String ScenarioNo) throws Throwable {
		...
		System.out.format("Thread ID - " + Thread.currentThread().getId());
			...
			Reporting.ReportLog ="";
			ConcurrentHashMap<String,inputData> allScenarios = RESTController.mad.getAllScenarios();
			...
			ConcurrentHashMap<String,ConcurrentHashMap<String,String>> allInputTestData = RESTController.mad.getAllInputTestData();
...
public void expected_should_match_with_actual() throws Throwable {
	...
	CopyOnWriteArrayList<StepResult> ArrStepResObj1 = testResultObj.getAllStepResult();
	System.out.println(testResultObj.getTestcaseid());
	for (StepResult eachResStep1 : ArrStepResObj1) {	
		System.out.println(eachResStep1.getStepResultLog());
		System.out.println(eachResStep1.getStepResultStatus());
	}
//	testResultObj.setResultLog(Reporting.ReportLog);
	//System.out.println(testResultObj.getAllStepResult().get(0).getStepResultLog());
	//ReportingStatus = reporting.WriteResultInFile(testResultObj);
	
	//System.out.println("Is result written?: " + ReportingStatus);
	RESTController.mad.getAllProcessedScenarios().put(testResultObj.getTestcaseid(), testResultObj);
}	
